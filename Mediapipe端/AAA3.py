...
# Hue Bar（手指 X 映射 Hue 0~1）——你可以保留，Unity 用 slider.value 作为 hue
HUEBAR_X = 100
HUEBAR_Y = 400
HUEBAR_W = 400
HUEBAR_H = 20
...
# ===============================
# 主程序（✅ 不维护 stage，只发 action 事件）
# ===============================
def main():
    cap = cv2.VideoCapture(0)
    font = cv2.FONT_HERSHEY_SIMPLEX

    trail_map = {"Left": deque(maxlen=TRAIL_LEN), "Right": deque(maxlen=TRAIL_LEN)}
    ...
    while True:
        ...
        # ======================================================
        # 1) SV 更新：左手掌心 -> S/V（一直更新，Unity 进入 ColorSelection 才用）
        #    x -> S, y -> V(上亮下暗)
        # ======================================================
        ...
        # ======================================================
        # 2) Hue 更新：左手 pinch 时，用 index x 映射 hue（一直更新也可以）
        #    如果你想只在特定阶段更新 Hue，那应该由 Unity gating，而不是 Python stage
        # ======================================================
        left_is_pinch = (left_pts is not None) and is_pinch(left_pts)
        if left_is_pinch and not slider_locked:
            x = left_pts[8][0]
            hue01 = clamp01((x - HUEBAR_X) / float(HUEBAR_W))
        ...
        # ======================================================
        # 4) 右手 Fist：cancel_step（同样建议加冷却，但先不加也行）
        # ======================================================
        ...
        if left_pts is not None:
            if is_fist(left_pts, "Left"):
                ...
                left_gesture = "grab"    # ✅ 统一：左手 pinch 就叫 grab（你 Unity 用它做抓取 gating）
        ...
        # ======================================================
        # 6) UDP 数据：V2 HSV-only（无 stage）
        # ======================================================
        control01 = val01  # 或 sat01，看你想怎么映射
        ...
        if now - last_print > 1.0:
            last_print = now
            print(f"[UDP] action={last_action} H={hue01:.2f} S={sat01:.2f} V={val01:.2f} L={left_gesture} R={right_gesture}")

        # ======================================================
        # GUI（调试）
        # ======================================================
        cv2.putText(frame, f"Action: {last_action}", (10, 25), font, 0.7, (235, 235, 235), 2)
        cv2.putText(frame, f"Hue: {hue01:.2f}  Sat: {sat01:.2f}  Val: {val01:.2f}", (10, 55), font, 0.6, (235, 235, 235), 2)
        cv2.putText(frame, f"Left:  {left_gesture} | Dir: {hand_direction['Left']}", (10, 85), font, 0.5, (235, 235, 235), 1)
        cv2.putText(frame, f"Right: {right_gesture} | Dir: {hand_direction['Right']}", (10, 110), font, 0.5, (235, 235, 235), 1)

        # Hue bar
        cv2.rectangle(frame, (HUEBAR_X, HUEBAR_Y), (HUEBAR_X + HUEBAR_W, HUEBAR_Y + HUEBAR_H), (235, 235, 235), 2)
        cv2.rectangle(frame, (HUEBAR_X, HUEBAR_Y),
                      (HUEBAR_X + int(hue01 * HUEBAR_W), HUEBAR_Y + HUEBAR_H),
                      (160, 130, 110), -1)
